...
class Anker_Connect_Wls_SearchForm_Widget extends SiteOrigin_Widget {
	...
	public function get_html_content( $instance, $args, $template_vars, $css_name ) {
		?>
        <wls-search-form<?php echo ! empty( $instance['query'] ) ? (' query="' . esc_attr($instance['query']) . '"') : ''; ?><?php echo ! empty( $instance['fields'] ) ? (' fields="' . esc_attr($instance['fields']) . '"') : ''; ?><?php echo ! empty( $instance['redirect'] ) ? (' redirect="' . esc_attr($instance['redirect']) . '"') : ''; ?><?php echo ! empty( $instance['class'] ) ? (' row-class="' . esc_attr($instance['class']) . '"') : ''; ?>></wls-search-form>
		<?php
	}
}
