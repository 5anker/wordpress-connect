...
class Anker_Connect_Elementor_Wls_SearchForm_Widget extends \Elementor\Widget_Base
{
	...
	protected function render($instance = [])
	{
		...
		<wls-search-form<?php echo !empty($settings['query']) ? (' query="' . esc_attr($settings['query']) . '"') : ''; ?><?php echo !empty($settings['fields']) ? (' fields="' . esc_attr($settings['fields']) . '"') : ''; ?><?php echo !empty($settings['redirect']) ? (' redirect="' . esc_attr($settings['redirect']) . '"') : ''; ?><?php echo !empty($settings['class']) ? (' row-class="' . esc_attr($settings['class']) . '"') : ''; ?>></wls-search-form>
		<?php
	}
}
